...
public extension CryoNet {
    ...
    /// 发起一个文件下载请求。
    ///
    /// 此方法用于从指定的 URL 下载文件，并支持断点续传、下载进度跟踪和拦截器处理。
    ///
    /// - Parameters:
    ///   - model: 包含下载目标 URL、目标路径和超时设置的 ``DownloadModel`` 实例。
    ///   - headers: 可选的额外 HTTP 请求头数组，将与 ``CryoNetConfiguration`` 中的基础请求头合并。默认为空数组。
    ///   - interceptor: 可选的自定义请求拦截器，如果提供，将覆盖 ``CryoNetConfiguration`` 中的默认拦截器。默认为 `nil`。
    ///
    /// - Returns: 一个 ``CryoStreamResult`` 实例，用于链式调用处理下载响应和进度。
    ///
    /// ### 使用示例
    /// ```swift
    ...
    /// )
    ///
    /// await cryoNet.download(downloadModel)
    /// ```
    ///
    /// - SeeAlso: ``DownloadModel``, ``CryoStreamResult``
    @available(macOS 10.15, iOS 13, *)
    func downloadFile(
        _ model: DownloadModel,
        progress: @escaping @Sendable (DownloadItem) -> Void = {_ in },
